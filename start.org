...
#+END_SRC
   ...
   =nrepl=, the Emacs client for the Clojure REPL, can be configured
   to be less annoying. We want to hide its special buffers.
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-c C-k") 'cnd/kill-current-buffer-file)
